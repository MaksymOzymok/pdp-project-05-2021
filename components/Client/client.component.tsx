import React from "react";
import Router from "next/router";
import Swal from "sweetalert2";
import axiosClient from "../../config/axios";
import styles from "./client.module.scss";

const Client = ({ data, clients, setClients }) => {
  const { name, company, email, id } = data;
  const handleDelete = () => {
    Swal.fire({
      title: "Ви впевнені, що хочете видалити клієнта?",
      text: "Відновити клієнта неможливо",
      icon: "warning",
      showCancelButton: true,
      confirmButtonText: "Так, підтверджую",
      cancelButtonText: "Ні, скасувати",
    }).then(async (result) => {
      if (result.value) {
        try {
          await axiosClient.delete(`users/${id}`);
          setClients(clients.filter((client) => client.id !== id));
          Swal.fire("Успіх!", "Клієнта видалено", "success");
        } catch (error) {
          console.log(error);
          Swal.fire("Упс...", "Щось пішло не так!", "error");
        }
      }
    });
  };
  const handleEdit = () => {
    Router.push({
      pathname: "/editclient/[id]",
      query: { id },
    });
  };
  return (
    <tr>
      <td className={styles.tableCell}>{name}</td>
      <td className={styles.tableCell}>{company}</td>
      <td className={styles.tableCell}>{email}</td>
      <td className={styles.manage}>
        <button
          type="button"
          className={styles.edit}
          onClick={() => handleEdit()}
        >
          <svg
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            viewBox="0 0 24 24"
          >
            <path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
          </svg>
        </button>
        <button
          type="button"
          onClick={() => handleDelete()}
          className={styles.delete}
        >
          <svg
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="2"
            viewBox="0 0 24 24"
          >
            <path d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
          </svg>
        </button>
      </td>
    </tr>
  );
};

export { Client };
